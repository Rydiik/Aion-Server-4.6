...
if (isset ($row["exp"])) {
	...
	elseif ($row["exp"] <= '1083018') {$exp = "16";}
	elseif ($row["exp"] <= '1401356') {$exp = "17";}
	elseif ($row["exp"] <= '1808613') {$exp = "18";}
	elseif ($row["exp"] <= '2314771') {$exp = "19";}
	elseif ($row["exp"] <= '2941893') {$exp = "20";}
	elseif ($row["exp"] <= '3769257') {$exp = "21";}
	elseif ($row["exp"] <= '4811154') {$exp = "22";}
	elseif ($row["exp"] <= '6110198') {$exp = "23";}
	elseif ($row["exp"] <= '7632340') {$exp = "24";}
	elseif ($row["exp"] <= '9377726') {$exp = "25";}
	elseif ($row["exp"] <= '11395643') {$exp = "26";}
	elseif ($row["exp"] <= '13731725') {$exp = "27";}
	elseif ($row["exp"] <= '16339413') {$exp = "28";}
	elseif ($row["exp"] <= '19378549') {$exp = "29";}
	elseif ($row["exp"] <= '23252749') {$exp = "30";}
	elseif ($row["exp"] <= '27675843') {$exp = "31";}
	elseif ($row["exp"] <= '32911197') {$exp = "32";}
	elseif ($row["exp"] <= '39197217') {$exp = "33";}
	elseif ($row["exp"] <= '47420762') {$exp = "34";}
	elseif ($row["exp"] <= '57899684') {$exp = "35";}
	elseif ($row["exp"] <= '70724362') {$exp = "36";}
	elseif ($row["exp"] <= '87641065') {$exp = "37";}
	elseif ($row["exp"] <= '107088757') {$exp = "38";}
	elseif ($row["exp"] <= '129885732') {$exp = "39";}
	elseif ($row["exp"] <= '157281282') {$exp = "40";}
	elseif ($row["exp"] <= '189342188') {$exp = "41";}
	elseif ($row["exp"] <= '227003751') {$exp = "42";}
	elseif ($row["exp"] <= '267317400') {$exp = "43";}
	elseif ($row["exp"] <= '310123925') {$exp = "44";}
	elseif ($row["exp"] <= '355885203') {$exp = "45";}
	elseif ($row["exp"] <= '404893687') {$exp = "46";}
	elseif ($row["exp"] <= '456755353') {$exp = "47";}
	elseif ($row["exp"] <= '511753757') {$exp = "48";}
	elseif ($row["exp"] <= '570232075') {$exp = "49";}
	elseif ($row["exp"] <= '632338545') {$exp = "50";}
	elseif ($row["exp"] <= '701655822') {$exp = "51";}
	elseif ($row["exp"] <= '776901823') {$exp = "52";}
	elseif ($row["exp"] <= '857160855') {$exp = "53";}
	elseif ($row["exp"] <= '941190930') {$exp = "54";}
	elseif ($row["exp"] <= '1037885487') {$exp = "55";}
	elseif ($row["exp"] <= '1149277979') {$exp = "56";}
	elseif ($row["exp"] <= '1276707285') {$exp = "57";}
	elseif ($row["exp"] <= '1420814969') {$exp = "58";}
	elseif ($row["exp"] <= '1584752478') {$exp = "59";}
	elseif ($row["exp"] <= '1823962406') {$exp = "60";}	
	elseif ($row["exp"] <= '2072828069') {$exp = "61";}
	elseif ($row["exp"] <= '2330507012') {$exp = "62";}
	elseif ($row["exp"] <= '2611618633') {$exp = "63";}
	elseif ($row["exp"] <= '3120186907') {$exp = "64";}
	elseif ($row["exp"] <= '3704748142') {$exp = "65";}
}


// Race
if (isset ($row["race"])) {
	if ($row["race"] == 'ELYOS') {$race = "<img src='img_top/ely.png' title='Elyos' />";}
	elseif ($row["race"] == 'ASMODIANS') {$race = "<img src='img_top/asmo.png' title='Asmodian' />";}
}

// Legions Rank
if (isset ($row["rank"])) {
	if ($row["rank"] == 'BRIGADE_GENERAL') {$rank_leg = "Brigadier General";}
	elseif ($row["rank"] == 'CENTURION') {$rank_leg = "Centurion";}
	elseif($row["rank"] == 'LEGIONARY') {$rank_leg = "Legionary";}
	else {$rank_leg = "Recruit";}
}


// Class
if (isset ($row["player_class"])) {
	if($row["player_class"] == 'WARRIOR') {$player_class = "<img src='img_top/warrior.png' title='Warrior'>";}
	elseif($row["player_class"] == 'GLADIATOR') {$player_class = "<img src='img_top/gladiator.png' title='Gladiator'>";}
	elseif($row["player_class"] == 'TEMPLAR') {$player_class = "<img src='img_top/templar.png' title='Templar'>";}
	elseif($row["player_class"] == 'SCOUT') {$player_class = "<img src='img_top/scout.png' title='Scout'>";}
	elseif($row["player_class"] == 'ASSASSIN') {$player_class = "<img src='img_top/assassin.png' title='Assassin'>";}
	elseif($row["player_class"] == 'RANGER') {$player_class = "<img src='img_top/ranger.png' title='Ranger'>";}
	elseif($row["player_class"] == 'MAGE') {$player_class = "<img src='img_top/mage.png' title='Ðœage'>";}
	elseif($row["player_class"] == 'SORCERER') {$player_class = "<img src='img_top/sorcerer.png' title='Sorcerer'>";}
	elseif($row["player_class"] == 'SPIRIT_MASTER') {$player_class = "<img src='img_top/spiritmaster.png' title='Spiritmaster'>";}
	elseif($row["player_class"] == 'PRIEST') {$player_class = "<img src='img_top/priest.png' title='Priest'>";}
	elseif($row["player_class"] == 'CLERIC') {$player_class = "<img src='img_top/cleric.png' title='Cleric'>";}
	elseif($row["player_class"] == 'CHANTER') {$player_class = "<img src='img_top/chanter.png' title='Chanter'>";}
	elseif($row["player_class"] == 'ENGINEER') {$player_class = "<img src='img_top/engineer.png' title='Engineer'>";}
	elseif($row["player_class"] == 'GUNNER') {$player_class = "<img src='img_top/gunner.png' title='Gunner'>";}
	elseif($row["player_class"] == 'RIDER') {$player_class = "<img src='img_top/rider.png' title='Rider'>";}
	elseif($row["player_class"] == 'ARTIST') {$player_class = "<img src='img_top/artist.png' title='Artist'>";}
	elseif($row["player_class"] == 'BARD') {$player_class = "<img src='img_top/bard.png' title='Bard'>";}
}


// Sex
if (isset ($row["gender"])) {
	if ($row["gender"] == 'MALE') {$gender = "<img src='img_top/male.png' title='Male' />";}
	elseif ($row["gender"] == 'FEMALE') {$gender = "<img src='img_top/female.png' title='Female' />";}
}
...
if (isset ($row["world_id"])) {
	...
	else if($row["world_id"] == 300260000) {$world_id = "Elementis Forest" ;}
	else if($row["world_id"] == 300270000) {$world_id = "Argent Manor" ;}
	else if($row["world_id"] == 300280000) {$world_id = "Rentus Base" ;}
	else if($row["world_id"] == 300310000) {$world_id = "Raksang" ;}
	...
	else if($row["world_id"] == 301010000) {$world_id = "Steel Rose Cargo (Solo)" ;}
	else if($row["world_id"] == 301020000) {$world_id = "Steel Rose Quarters (Solo)" ;}
	else if($row["world_id"] == 301030000) {$world_id = "Steel Rose Cargo (Group)" ;}
	else if($row["world_id"] == 301040000) {$world_id = "Steel Rose Quarters (Group)" ;}
	else if($row["world_id"] == 301050000) {$world_id = "Steel Rose Deck" ;}
	...
	else if($row["world_id"] == 301140000) {$world_id = "Danuar Sanctuary" ;}
	else if($row["world_id"] == 301160000) {$world_id = "Rukibuki Circus" ;}
	else if($row["world_id"] == 301170000) {$world_id = "Legion Idgel Research Center" ;}
	else if($row["world_id"] == 301180000) {$world_id = "Legion Void Cube" ;}
	else if($row["world_id"] == 301190000) {$world_id = "Legion Danuar Mysticarium" ;}
	...
	else if($row["world_id"] == 600020000) {$world_id = "Sarpan";}
	else if($row["world_id"] == 600030000) {$world_id = "Tiamaranta";}
	else if($row["world_id"] == 600040000) {$world_id = "Tiamaranta's Eye";}
	else if($row["world_id"] == 600050000) {$world_id = "Katalam";}
	else if($row["world_id"] == 600060000) {$world_id = "Danaria";}
	else if($row["world_id"] == 600070000) {$world_id = "Idian Depths";}
...
// Abyss Rank
if (isset ($row["rank"])) {
	if($row["rank"] == '1') {$rank = "Soldier, Rank 9";}
	elseif($row["rank"] == '2') {$rank = "Soldier, Rank 8";}
	elseif($row["rank"] == '3') {$rank = "Soldier, Rank 7";}
	elseif($row["rank"] == '4') {$rank = "Soldier, Rank 6";}
	elseif($row["rank"] == '5') {$rank = "Soldier, Rank 5";}
	elseif($row["rank"] == '6') {$rank = "Soldier, Rank 4";}
	elseif($row["rank"] == '7') {$rank = "Soldier, Rank 3";}
	elseif($row["rank"] == '8') {$rank = "Soldier, Rank 2";}
	elseif($row["rank"] == '9') {$rank = "Soldier, Rank 1";}
	elseif($row["rank"] == '10') {$rank = "Army 1-Star Officer (*)";}
	elseif($row["rank"] == '11') {$rank = "Army 2-Star Officer (**)";}
	elseif($row["rank"] == '12') {$rank = "Army 3-Star Officer (***)";}
	elseif($row["rank"] == '13') {$rank = "Army 4-Star Officer (****)";}
	elseif($row["rank"] == '14') {$rank = "Army 5-Star Officer (*****)";}
	elseif($row["rank"] == '15') {$rank = "General";}
	elseif($row["rank"] == '16') {$rank = "Great General";}
	elseif($row["rank"] == '17') {$rank = "Commander";}
	elseif($row["rank"] == '18') {$rank = "Governor";}
}