...
$query = "SELECT players.name AS nick, legions.name, legions.level, players.race, contribution_points FROM legions LEFT JOIN legion_members ON legions.id = legion_members.legion_id LEFT JOIN players ON legion_members.player_id = players.id WHERE legions.id='$id' AND legion_members.rank='BRIGADE_GENERAL'";
...
if (isset($_POST['submit'])) {
    if (isset($_POST['name']) && isset($_POST['level'])) {
        ...
        $query = "UPDATE legions SET name = '$name', level = '$level' WHERE id = '$id'";
